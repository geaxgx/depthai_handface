...
def send_result_hand(pd_inf, nb_lm_inf, lm_score=0, handedness=0, rect_center_x=0, rect_center_y=0, rect_size=0, rotation=0, rrn_lms=0, sqn_lms=0, world_lms=0):
    result = dict([("pd_inf", pd_inf), ("nb_lm_inf", nb_lm_inf), ("lm_score", [lm_score]), ("handedness", [handedness]), ("rotation", [rotation]),
            ...
            ("rrn_lms", [rrn_lms]), ('sqn_lms', [sqn_lms]), ('world_lms', [world_lms])])
    send_result(result)
...
while True:
    ...
    if send_new_frame_to_branch == 1: # Routing frame to pd branch
        ...
        if valid_outputs ==0 or box_size < 0:
    ...
    nb_lm_inf += 1
    ${_TRACE2} ("Hand manager sent config to pre_lm manip")
    ...
    if lm_score > ${_lm_score_thresh}:
        ...
        # Send result to host
        send_result_hand(send_new_frame_to_branch==1, nb_lm_inf, lm_score, handedness, sqn_rr_center_x, sqn_rr_center_y, sqn_rr_size, rotation, rrn_lms, sqn_lms, world_lms)
